...
(define (my-test)
  (sleep 3)
  (press-keys fd (list KEY-Q KEY-W)))

(define (fenrir3)
  (sleep 3)
  (press-key fd KEY-RIGHT)
  (press-key fd KEY-DOWN)
  (press-keys fd (list KEY-DOWN KEY-RIGHT))
  (press-keys fd (list KEY-W KEY-E)))
...
(define (hairball)
  (sleep 3)
  (press-key fd KEY-DOWN)
  (press-keys fd (list KEY-DOWN KEY-LEFT))
  (press-key fd KEY-LEFT)
  (press-keys fd (list KEY-R KEY-T)))


; Push some buttons in Racket!
(define fd (setup_uinput_device))
(sleep 1)
(emit fd ev-key key-leftmeta 1)
(emit fd ev-syn syn-report 0)
(emit fd ev-key key-leftmeta 0)
(emit fd ev-syn syn-report 0)
(sleep 1)
(emit fd ev-key key-leftmeta 1)
(emit fd ev-syn syn-report 0)
